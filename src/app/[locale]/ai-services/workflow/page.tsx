...
import SegmentCards, { type SegmentCardItem, type SegmentCardLabels } from '@/components/SegmentCards';
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return { title: dict.aiWorkflowPage.meta.title, description: dict.aiWorkflowPage.meta.description };
}

export default async function WorkflowAutomation({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  if (!locales.includes(locale as Locale)) notFound();
  const dict = await getDictionary(locale as Locale);
  const t = dict.aiWorkflowPage;
  const segmentItems: SegmentCardItem[] = t.segments.items;
  const segmentLabels: SegmentCardLabels = t.segmentLabels;
    ...
    <>
      ...
      <section className="relative py-16 sm:py-24">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="flex justify-center mb-4"><WorkflowIcon size={48} /></div>
      ...
      {/* Industry Segments with integrated Workflows */}
      <section className="pb-20 sm:pb-28">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <SectionHeading title={t.segments.title} />
          <SegmentCards items={segmentItems} labels={segmentLabels} />
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-3 gap-6">
            {t.techStack.items.map((tech, i) => (
              <div key={i} className="p-6 rounded-xl bg-surface/30 border border-white/5 text-center">
                <h3 className="text-lg font-bold text-accent-blue mb-2">{tech.name}</h3>
